...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Actualizar nombre de usuario en el botón de dropdown de navegación (si existe)
    const username = localStorage.getItem("username");
    if (username) {
        const drpBtn = document.querySelector(".dropdown-toggle");
        if (drpBtn) {
            drpBtn.innerHTML = `👤 ${username}`;
        }
    }
});
